    ...
    const std::vector<SpringNode*>& failed_nodes)
{
    ...
    if (failed_nodes.empty() || !world) {
        return clusters;
    }

    std::unordered_set<const SpringNode*> visited;
    ...
    for (const auto* node : failed_nodes) {
        ...
        std::queue<const SpringNode*> frontier;
        frontier.push(node);
        visited.insert(node);

        while (!frontier.empty()) {
            const SpringNode* current = frontier.front();
            frontier.pop();

            cluster.voxel_positions.push_back(current->position);
            if (cluster.dominant_material == 0) {
                cluster.dominant_material = current->material_id;
            }

            for (const auto* neighbor : current->neighbors) {
                if (!neighbor || !neighbor->has_failed || visited.count(neighbor)) {
                    continue;
                }
